...
2. Once your AVLTree is functional, modify the rest of the methods to to use
your AVLTree for storage and lookup. For example, think about what methods to use instead of  `set`.

3. The rest of the restrictions and correctness requirements for HWs 01 and 04
remain. Correctness testing (on lookups once the AVL tree is functional)
should be the same as well.

4. Time your new implementation using commandline redirection of `in.txt`
test file from HW01.
That is, issue the
following command at the prompt:
`time java DriverHW08 -i uszipcodes.csv ziplocs.csv < in.txt > out.txt`
