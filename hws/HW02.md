...
due_date: 2023-10-03 23:59:00EDT
...
The LL must support all the methods from Lab03, including the DoubleLinkedList methods.
...
Write a method called `rank()` based on the specification below:
...
*
* @return array indicating the the rank of the name that year, the number of babies given that name that year (for that gender), percentage - the percentage of babies given that name that year (for that gender)
*/
public int yearStats(String name, int year, char gender)
```
...
*/
public int totalStats(String name, char gender)
```
...
will print out the rank, number and percentages of the
female name Dianna used in 1990, 2000 as well as the combined statistics of these
two years, as shown below (398 is the alphabetically ranked order of the name `Dianna`
...
add a -f or -m to indicate the gender they are searching for.
 ...
 For example, 
```
java Main -f Dianna -m Adam names1990.csv names2000.csv
```
or 

```
java Main -f Dianna -f Aline names1990.csv names2000.csv 
```
...
The csv files are all available on Piazza. Download the zip file called `hw02_data.zip`.
It contains these csv files as well as some tests.
...
- For a bigger sanity check, look in the tests. 
Multiple expected output files are provided for you there, as well as a bash shell
script to automate testing. Start by reading the README in the subdirectory and follow instructions there.
...
3. How do you organize the storage of the yearly statistics per name versus the
totals?
